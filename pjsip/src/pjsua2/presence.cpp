...
void Buddy::create(Account &account, const BuddyConfig &cfg) throw(Error)
{
    ...
    PJSUA2_CHECK_EXPR( pjsua_buddy_add(&pj_cfg, &id, -1) );
...
{
    PJSUA2_CHECK_EXPR( pjsua_buddy_subscribe_pres(id, subscribe, -1) );
}
...
{
    PJSUA2_CHECK_EXPR( pjsua_buddy_update_pres(id, -1) );
}
