...
static void subscribe_buddy_presence(pjsua_buddy_id buddy_id, pj_int32_t expires);
...
PJ_DEF(pj_status_t) pjsua_buddy_add( const pjsua_buddy_config *cfg,
				     pjsua_buddy_id *p_buddy_id, pj_int32_t expires)
{
    pjsip_name_addr *url;
    pjsua_buddy *buddy;
	...
	*p_buddy_id = index;
    ...
    pjsua_buddy_subscribe_pres(index, cfg->subscribe, expires);
...
PJ_DEF(pj_status_t) pjsua_buddy_del(pjsua_buddy_id buddy_id)
{
    ...
    pjsua_buddy_subscribe_pres(buddy_id, PJ_FALSE, -1);
...
PJ_DEF(pj_status_t) pjsua_buddy_subscribe_pres( pjsua_buddy_id buddy_id,
						pj_bool_t subscribe, pj_int32_t expires)
{
    struct buddy_lock lck;
    pj_status_t status;
    ...
    pjsua_buddy_update_pres(buddy_id, expires);
 ...
 */
PJ_DEF(pj_status_t) pjsua_buddy_update_pres(pjsua_buddy_id buddy_id, pj_int32_t expires)
{
    struct buddy_lock lck;
    pj_status_t status;
    ...
    subscribe_buddy_presence(buddy_id, expires);
				       ...
				       const pjsua_msg_data *msg_data)
{
	...
	if (b->monitor && b->sub == NULL) {
	    ...
	    subscribe_buddy_presence(buddy_id, -1);
	}
    }
...
static void buddy_timer_cb(pj_timer_heap_t *th, pj_timer_entry *entry)
{
    ...
    entry->id = PJ_FALSE;
    pjsua_buddy_update_pres(buddy->index, -1);
}
...
static void subscribe_buddy_presence(pjsua_buddy_id buddy_id, pj_int32_t expires)
{
    ...
    status = pjsip_pres_initiate(buddy->sub, expires, &tdata);
    if (status != PJ_SUCCESS) {
	if (buddy->dlg) pjsip_dlg_dec_lock(buddy->dlg);
	if (buddy->sub) {
